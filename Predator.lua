...
startMessage = "~r~¦~s~WELCOME ~r~[ ~s~1.1 ~r~]~n~👤 USER → ~r~[~s~ " ..GetPlayerName(PlayerId()).. " ~r~]"	
...
motd5 = "Stay Fast https://fastgang.de/dc/"
...
function MIOddhwuie.SetTheme(id, theme)
    ...
    elseif theme == "skid" then
        ...
        MIOddhwuie.SetMenuX(id, .75)
        MIOddhwuie.SetMenuY(id, .1)
        MIOddhwuie.SetMenuWidth(id, 0.20)-- 0.23
...
Citizen.CreateThread(function()
    ...
    MIOddhwuie.SetSubTitle('skid', 'sprchblse.de <- FREE MENUS')
    ...
    while true do
        ...
        if MIOddhwuie.IsMenuOpened('skid') then
            if MIOddhwuie.MenuButton('Player Options', 'player') then
