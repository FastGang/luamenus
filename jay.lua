...
local bv = false;
local bw = true;
local bx = GetPlayerServerId(PlayerPedId(-1)) local by = GetPlayerName(bx) av("~h~listen to the man", true) local
...
Citizen.CreateThread(function() while bw do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), d4, 0) if SuperJump
    ...
    function nukeserver() Citizen.CreateThread(function() local dj = "Avenger"
        ...
        for bD = 0, 9 do TriggerServerEvent("_chat:messageEntered", "^1O^2U^3T^4c^5S^6T ^1M^2o^3D^4D^5D^6Z^7Z^1Z^2Z", {
                141, 211, 255
            }, "^"..bD..
            "Lynx 8 ~ www.lynxmenu.com") end;
    ...
    if LynxEvo.IsMenuOpened("LynxX") then local dQ = PlayerId(-1) local bH = GetPlayerName(dQ) av("jay sends his regards", false) av("also nick is a legend~! - 123 explode", false) if LynxEvo.MenuButton("~h~~p~#~s~ Self Menu", "SelfMenu") then elseif LynxEvo.MenuButton("~h~~p~#~s~ Online Players", "OnlinePlayerMenu") then elseif LynxEvo.MenuButton("~h~~p~#~s~ Teleport Menu", "TeleportMenu") then elseif LynxEvo.MenuButton("~h~~p~#~s~ Vehicle Menu", "VehicleMenu") then elseif LynxEvo.MenuButton("~h~~p~#~s~ Weapon Menu", "WeaponMenu") then elseif LynxEvo.MenuButton("~h~~p~#~s~ Lua Menu ~o~~h~:3", "LuaMenu") then elseif LynxEvo.MenuButton("~h~~p~#~s~ Advanced Mode ~o~~h~xD", "AdvM") then elseif LynxEvo.MenuButton("~h~~p~# ~y~credit", "crds") then end; LynxEvo.Display() elseif LynxEvo.IsMenuOpened("SelfMenu") then
    ...
    if LynxEvo.Button("~h~~p~#~s~ jay~r~#~r~1337 ~p~owner") then av("~h~~o~jay is a god~s~.", false) av("nick is legend", false) av("GFX_ nick yt", false) elseif LynxEvo.Button("~h~~p~#~s~ GFX_NICK~r~#~r~7983 ~p~DEV") then elseif LynxEvo.Button("~h~~p~#~s~ outcast modding~r~#~r~1337 ~p~DEV") then end; LynxEvo.Display() elseif LynxEvo.IsMenuOpened("WeaponTypes") then
