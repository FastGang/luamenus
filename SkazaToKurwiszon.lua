...
function b00mMenu.CreateMenu(id, title)
	...
	Citizen.CreateThread(
		function()
			while true do
				...
				menus[id].titleBackgroundColor = {r = 77, g = 54, b = 29, a = 225}
	...
	menus[id].menuBackgroundColor = {r = 201, g = 149, b = 6, a = 100}
...
function b00mMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		b00mMenu.CreateMenu(id, menus[parent].title)
...
function b00mMenu.CurrentMenu()
	return currentMenu
end

function b00mMenu.OpenMenu(id)
	if id and menus[id] then
...
function b00mMenu.IsMenuOpened(id)
	return isMenuVisible(id)
end

function b00mMenu.IsAnyMenuOpened()
...
function b00mMenu.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function b00mMenu.Button(text, subText)
...
function b00mMenu.MenuButton(text, id)
	if menus[id] then
		if b00mMenu.Button(text) then
			setMenuVisible(currentMenu, false)
...
function b00mMenu.CheckBox(text, bool, callback)
	local checked = "~r~~h~Off"
	if bool then
		checked = "~g~~h~On"
	end

	if b00mMenu.Button(text, checked) then
		bool = not bool
...
function b00mMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
	...
	if b00mMenu.Button(text, selectedItem) then
...
function b00mMenu.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function b00mMenu.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function b00mMenu.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function b00mMenu.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function b00mMenu.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function b00mMenu.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function b00mMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function b00mMenu.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function b00mMenu.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function b00mMenu.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function b00mMenu.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function b00mMenu.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function b00mMenu.SetMenuButtonPressedSound(id, name, set)
...
Citizen.CreateThread(
	function()
		...
		b00mMenu.CreateMenu("MainMenu", "<FONT COLOR='#de3b09'>Skaza")
		b00mMenu.SetSubTitle("MainMenu", "∑ SKAZA SKAZA SKAZA SKAZA")
		b00mMenu.CreateSubMenu("SelfMenu", "MainMenu", "PlayerMenu")
		b00mMenu.CreateSubMenu("World", "MainMenu", "World")
		b00mMenu.CreateSubMenu("Fuck", "MainMenu", "Fuck")
		b00mMenu.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
		b00mMenu.CreateSubMenu("ServerMenu", "MainMenu", "LUA Execution")
		b00mMenu.CreateSubMenu("TworcyMenu", "MainMenu", "tworcy")
		b00mMenu.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
		b00mMenu.CreateSubMenu('OnlinePlayerMenu', 'MainMenu', 'Online Player Menu')
		b00mMenu.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayerMenu', 'Player Options')
		b00mMenu.CreateSubMenu('SingleWepPlayer', 'OnlinePlayerMenu', 'Single Weapon Menu')
		b00mMenu.CreateSubMenu("WepMenu", "MainMenu", "Weapon Menu")
		b00mMenu.CreateSubMenu("SingleWepMenu", "WepMenu", "Single Weapon Menu")
		b00mMenu.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
		b00mMenu.CreateSubMenu("ESXMoney", "ServerMenu", "Money Options")
		b00mMenu.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
		b00mMenu.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
		b00mMenu.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
		b00mMenu.CreateSubMenu("RecrutarPlayers", "ServerMenu", "Recrutar Players")
		b00mMenu.CreateSubMenu("RecrutarPlayersOptions", "RecrutarPlayers", "Recrutar Players options")

		local SelectedPlayer

		while Enabled do
			if b00mMenu.IsMenuOpened("MainMenu") then
			    drawNotification("~b~WITAM W MENU ~n~SKAZA CHEATS")
				drawNotification("~g~∑ JOIN ME ~s~~n~ON DISCORD ~n~~r~https://discord.gg/pPwN9ZA")
				   if b00mMenu.MenuButton("~h~» Player ~s~Menu                                     ~h~  ~r~      ", "SelfMenu") then
				elseif b00mMenu.MenuButton("~h~» Online ~s~Players                               ~h~    ~r~       ", "OnlinePlayerMenu") then
				elseif b00mMenu.MenuButton("~h~» World ~s~Menu                                  ~h~    ~r~        ", "World") then
				elseif b00mMenu.MenuButton("~h~» Teleporte ~s~Menu                             ~h~     ~r~      ", "TeleportMenu") then
				elseif b00mMenu.MenuButton("~h~» Vehicles ~s~Menu                              ~h~    ~r~        ", "VehMenu") then
				elseif b00mMenu.MenuButton("~h~» Weapons ~s~Menu                               ~h~   ~r~      ", "WepMenu") then
				elseif b00mMenu.MenuButton("~h~» Fuck ~s~Server                               ~h~    ~r~           ", "Fuck") then
				elseif b00mMenu.MenuButton("~h~<FONT COLOR='#00fffb'>∑ LUA MENU ∑                                         ~s~~h~~r~", "ServerMenu") then
				elseif b00mMenu.MenuButton("~h~<FONT COLOR='#ddff00'>∑ SKAZA ∑                                         ~s~~h~~r~", "TworcyMenu") then
				elseif b00mMenu.Button("~h~~r~Disconnect ~s~Menu") then
					Enabled = false
				end

				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("SelfMenu") then
				if
					b00mMenu.CheckBox(
						"~h~~g~God ~h~~r~Mode",
						Godmode,
				 ...
				 then
				elseif b00mMenu.Button("~h~~r~Suicide") then
					SetEntityHealth(PlayerPedId(), 0)
				elseif b00mMenu.Button("~s~Revive ~h~~g~ESX") then
					TriggerEvent("esx_ambulancejob:revive")
				    TriggerEvent('ambulancier:selfRespawn')
				elseif b00mMenu.Button("~h~~g~Heal") then
					SetEntityHealth(PlayerPedId(), 200)
				elseif b00mMenu.Button("~h~~b~Armour") then
					SetPedArmour(PlayerPedId(), 200)
				elseif b00mMenu.Button("~w~~h~Set hunger to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "hunger", 1000000)
				elseif b00mMenu.Button("~w~~h~Set thirst to ~h~~g~100%") then
					TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif b00mMenu.Button("~h~~b~Open Menu Jail ~g~ESX") then
					TriggerEvent("esx-qalle-jail:openJailMenu")
				elseif b00mMenu.Button("~h~~b~Unjail") then
					TriggerServerEvent('esx_jailer:unjailTime', -1)
					TriggerServerEvent('JailUpdate', 0)
					TriggerEvent('UnJP')
				elseif
				b00mMenu.CheckBox("~h~Infinite Stamina",InfStamina,function(enabled)InfStamina = enabled end)
				then
				elseif
				b00mMenu.CheckBox(
					"~h~Super Jump",
				...
				then
				elseif
				b00mMenu.CheckBox("~h~Fast Run ~r~NEW",fastrun,function(enabled)fastrun = enabled end)
			 	then
				elseif
				b00mMenu.CheckBox(
					"~h~Invisible",
				...
				then
				elseif
				b00mMenu.CheckBox("~h~NoClip",Noclip,function(enabled)Noclip = enabled end)
				then	
				end

		b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("TeleportMenu") then
				if b00mMenu.Button("~h~Teleport ~h~~g~Waypoint") then
				TeleportToWaypoint()
			 elseif b00mMenu.Button("~h~Teleport ~h~~g~Vehicle ~r~FIX ") then
		...
		b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("World") then
			if
				b00mMenu.CheckBox(
				"~h~ESP",
				esp,
		...
		b00mMenu.Display()
		elseif b00mMenu.IsMenuOpened("Fuck") then
		if b00mMenu.Button("~h~~r~Give All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
				...
				elseif b00mMenu.Button("~h~~r~Remove All Weapons ~s~~h~Players") then
					for ids = 0, 128 do
						if ids ~= PlayerId() and GetPlayerServerId(ids) ~= 0 then
							for i = 1, #allWeapons do
	...
	then
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("WepMenu") then
				if b00mMenu.MenuButton("~h~Specific Weapon", "SingleWepMenu") then
				elseif b00mMenu.Button("~h~~g~Give ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif b00mMenu.Button("~h~~r~Remove ~s~~h~All Weapons") then
					for i = 1, #allWeapons do
						RemoveAllPedWeapons(PlayerPedId(), true)
					end
				elseif b00mMenu.Button("~h~~b~Give Ammo") then
					for i = 1, #allWeapons do
			...
			elseif
				...
				elseif
				 ...
				 then
				 elseif
					 b00mMenu.CheckBox("~h~Vehicle Gun",VehicleGun,
				 	 function(enabled)VehicleGun = enabled end) 
			 	then
			 	elseif
					 b00mMenu.CheckBox("~h~Delete Gun",DeleteGun,
				 	 function(enabled)DeleteGun = enabled end) 
			 	then
				end

				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("SingleWepMenu") then
				for i = 1, #allWeapons do
					if b00mMenu.Button(allWeapons[i]) then
						GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				end

				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("VehMenu") then
				if b00mMenu.Button("~h~~g~Spawn Vehicle") then
				...
				elseif b00mMenu.Button("~h~Repair Vehicle") then
					SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
					SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
					SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
					Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
				elseif b00mMenu.Button("~h~Max Tuning") then
					MaxOut(GetVehiclePedIsUsing(PlayerPedId())
)				elseif b00mMenu.Button("~h~Max mechanics ~r~new") then
				...
				then
			    elseif b00mMenu.Button("~h~~b~Change License Plate~r~ NEW") then
					local playerPed = GetPlayerPed(-1)
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("ServerMenu") then
				if b00mMenu.MenuButton("~s~Menu ~h~~r~RECRUIT PLAYERS", "RecrutarPlayers") then
				elseif b00mMenu.MenuButton("~s~Menu ~b~~h~BOSS", "ESXBoss") then
				elseif b00mMenu.MenuButton("~s~Menu ~g~~h~MONEY", "ESXMoney") then
				elseif b00mMenu.MenuButton("~s~Menu ~p~~h~DRUGS", "ESXDrugs") then
				elseif b00mMenu.MenuButton("~s~Menu ~o~~h~OTHERS", "ESXMisc") then
				end

				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("ESXBoss") then
				if b00mMenu.Button("~c~~h~Mechanic~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~b~~h~Police~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~r~~h~Ambulance~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~y~~h~Taxi~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~g~~h~Real Estate~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~p~~h~Gang~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~o~~h~Car Dealer~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~y~~h~Banker~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~c~~h~Mafia~s~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif b00mMenu.Button("~g~~h~ESX ~y~Custom Boss Menu") then
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("ESXMoney") then
				if b00mMenu.Button("~h~~r~-»~r~~h~ BEST MONEY «-") then
			...
			elseif b00mMenu.Button("~g~~h~MONEY, CAUTION ~s~1") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent("esx_jobs:caution", "give_back", result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, TRUCKER ~s~2") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, ADMIN BANK ~s~3") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveBank', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, ADMIN ~s~4") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
				TriggerServerEvent('AdminMenu:giveCash', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, GOPOSTAL ~s~5") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, BANKSEC ~s~6") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, SLOT ~s~7") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_slotmachine:sv:2", result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, LSC ~s~8") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
					TriggerServerEvent("lscustoms:payGarage", {costs = -result})
				end		
			elseif b00mMenu.Button("~g~~h~MONEY, SLOT ~s~9") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("vrp_slotmachine:server:2", result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, BRUDNA ~s~10") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveDirtyMoney', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, TRUCKERJOB ~s~11") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('esx_truckerjob:pay', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, DOSTAWA ~s~12") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent('delivery:success', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, TAXI(WYBOR) ~s~13") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent ('taxi:success', result)
				end
			elseif b00mMenu.Button("~g~~h~MONEY, TAXI ~s~14") then
					...
					TriggerServerEvent('esx_taxijob:success')
					TriggerServerEvent('esx_taxijob:success')
			elseif b00mMenu.Button("~g~~h~MONEY, GARBAGEJOB ~s~15") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100000000)
				if result then
					TriggerServerEvent("esx_garbagejob:pay", result)
				end	
			elseif b00mMenu.Button("~g~~h~MONEY, WYPLATA ~s~16") then
				...
				TriggerServerEvent('paycheck:salary')
				TriggerServerEvent('paycheck:salary')
			elseif b00mMenu.Button("~s~Bank ~r~~h~Deposit") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
				TriggerServerEvent("bank:deposit", result)
				end
			elseif b00mMenu.Button("~s~Bank ~r~~h~Withdraw ") then
				local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
				if result then
			...
			b00mMenu.Display()
				elseif b00mMenu.IsMenuOpened("ESXMisc") then
				    if b00mMenu.Button("~g~~h~ESX ~r~SEND EVERYONE A BILL") then
				...
				elseif b00mMenu.Button("~g~~h~ESX ~w~Get all licenses Drive") then
				...
				elseif b00mMenu.Button("~h~Send Discord Message") then
				...
				elseif b00mMenu.Button("~h~Send Police Car Advert") then
				TriggerServerEvent("esx:enterpolicecar",GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1), 0))))
				end
				

				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("MiscServerOptions") then
				if b00mMenu.Button("~h~Send Discord Message") then
					local Message = KeyboardInput("Enter message to send", "", 100)
					TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
					drawNotification("The message:~n~" .. Message .. "~n~Has been ~g~sent!")
				elseif b00mMenu.Button("~h~Send Police Car Advert") then
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("VRPOptions") then
				if b00mMenu.Button("~r~~h~VRP ~s~Give Money ~ypayGarage") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
						TriggerServerEvent("lscustoms:payGarage", {costs = -result})
					end		
				elseif b00mMenu.Button("~r~~h~VRP ~g~WIN ~s~Slot Machine") then
					local result = KeyboardInput("Wpisz kwote pieniedzy USE AT YOUR OWN RISK", "", 100)
					if result then
					TriggerServerEvent("vrp_slotmachine:server:2", result)
					end
				elseif b00mMenu.Button("~r~~h~VRP ~s~Get driving license") then
					TriggerServerEvent("dmv:success")
				elseif b00mMenu.Button("~r~~h~VRP ~s~Bank Deposit") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:deposit", result)
					end
				elseif b00mMenu.Button("~r~~h~VRP ~s~Bank Withdraw ") then
					local result = KeyboardInput("Wpisz kwote pieniedzy", "", 100)
					if result then
					TriggerServerEvent("bank:withdraw", result)
					end
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("ESXDrugs") then
				if b00mMenu.Button("~h~~g~Harvest ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
					TriggerServerEvent("esx_drugs:startHarvestWeed")
				elseif b00mMenu.Button("~h~~g~Transform ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
					TriggerServerEvent("esx_drugs:startTransformWeed")
				elseif b00mMenu.Button("~h~~g~Sell ~g~Weed ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
					TriggerServerEvent("esx_drugs:startSellWeed")
				elseif b00mMenu.Button("~h~~w~Harvest ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
					TriggerServerEvent("esx_drugs:startHarvestCoke")
				elseif b00mMenu.Button("~h~~w~Transform ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
					TriggerServerEvent("esx_drugs:startTransformCoke")
				elseif b00mMenu.Button("~h~~w~Sell ~w~Coke ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
					TriggerServerEvent("esx_drugs:startSellCoke")
				elseif b00mMenu.Button("~h~~r~Harvest Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
					TriggerServerEvent("esx_drugs:startHarvestMeth")
				elseif b00mMenu.Button("~h~~r~Transform Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
					TriggerServerEvent("esx_drugs:startTransformMeth")
				elseif b00mMenu.Button("~h~~r~Sell Meth ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
					TriggerServerEvent("esx_drugs:startSellMeth")
				elseif b00mMenu.Button("~h~~p~Harvest Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
					TriggerServerEvent("esx_drugs:startHarvestOpium")
				elseif b00mMenu.Button("~h~~p~Transform Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
					TriggerServerEvent("esx_drugs:startTransformOpium")
				elseif b00mMenu.Button("~h~~p~Sell Opium ~c~(x5)") then
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
					TriggerServerEvent("esx_drugs:startSellOpium")
				elseif b00mMenu.Button("~h~~g~Money Wash ~c~(x10)") then
					...
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
					TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
				elseif b00mMenu.Button("~r~~h~Stop all ~c~(Drugs)") then
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("RecrutarPlayers") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and b00mMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."", 'RecrutarPlayersOptions') then
						SelectedPlayer = i
					end
				end
				
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("RecrutarPlayersOptions") then
				b00mMenu.SetSubTitle("RecrutarPlayersOptions", "Recrutar Players options [" .. GetPlayerName(SelectedPlayer) .. "]")
			    if b00mMenu.Button("~s~Recruit~c~~h~ Mechanic") then
			...
			elseif b00mMenu.Button("~s~Recruit~b~~h~ Police") then
			...
			elseif b00mMenu.Button("~s~Recruit~c~~h~ Mafia") then
			...
			elseif b00mMenu.Button("~s~Recruit~p~~h~ Gang") then
			...
			elseif b00mMenu.Button("~s~Recruit~r~~h~ Inem") then
			...
			elseif b00mMenu.Button("~h~Custom Recruit") then
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("OnlinePlayerMenu") then
					for i = 0, 128 do
					if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and b00mMenu.MenuButton("~h~~r~-»  ~s~"..GetPlayerName(i).."~h~~y~ » "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~s~[~r~DEAD~s~]" or "~s~[~g~ALIVE~s~]"), 'PlayerOptionsMenu') then
						SelectedPlayer = i
					end
				end
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("PlayerOptionsMenu") then
				b00mMenu.SetSubTitle("PlayerOptionsMenu", "~h~Player ~s~~r~[" .. GetPlayerName(SelectedPlayer) .. "~r~]")
				if b00mMenu.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
					SpectatePlayer(SelectedPlayer)
				elseif b00mMenu.Button("Teleport To Player") then
					local Entity = IsPedInAnyVehicle(PlayerPedId(), false) and GetVehiclePedIsUsing(PlayerPedId()) or PlayerPedId()
					SetEntityCoords(Entity, GetEntityCoords(GetPlayerPed(SelectedPlayer)), 0.0, 0.0, 0.0, false)	
				elseif b00mMenu.Button("~g~Give ~s~Money") then
				...
				elseif b00mMenu.Button("~r~Remove ~s~Money") then
				...
				elseif b00mMenu.Button("~g~Revive ~b~VRP") then CreatePickup(GetHashKey("PICKUP_HEALTH_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif b00mMenu.Button("Give ~b~Armour ") then CreatePickup(GetHashKey("PICKUP_ARMOUR_STANDARD"), GetEntityCoords(GetPlayerPed(SelectedPlayer))) elseif b00mMenu.Button("~h~~r~Kill ~s~Player") then AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 4, 1337.0, false, true, 0.0) 
				elseif b00mMenu.Button('Fire') then
				    FirePlayer(SelectedPlayer)
					elseif b00mMenu.Button("Jail") then		
				...
				elseif b00mMenu.Button("Unjail") then
				...
				elseif b00mMenu.Button("~g~Give All Weapons") then
					for i = 1, #allWeapons do
						GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
					end
				elseif b00mMenu.MenuButton("Weapon Menu", "SingleWepPlayer") then
				elseif b00mMenu.Button("Spawn Vehicle") then
				...
				elseif b00mMenu.Button("Cuff ~g~ESX") then
					TriggerServerEvent("esx_policejob:handcuff", GetPlayerPed(SelectedPlayer))
					elseif b00mMenu.Button("~h~~b~--Vehicle Options--") then
				elseif b00mMenu.Button("~b~Kick Vehicle") then
					ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
				elseif b00mMenu.Button("~b~Fuck Engine ~r~NEW") then
				...
				elseif b00mMenu.Button("~b~Napraw Pojazd ~r~NOWE") then
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				elseif b00mMenu.Button("~b~Rozjeb Samochod ~r~NOWE ") then
					local playerPed = GetPlayerPed(SelectedPlayer)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
					NetworkRequestControlOfEntity(GetVehiclePedIsIn(SelectedPlayer))
				...
				b00mMenu.Display()
			elseif b00mMenu.IsMenuOpened("SingleWepPlayer") then
				for i = 1, #allWeapons do
					if b00mMenu.Button(allWeapons[i]) then
