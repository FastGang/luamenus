...
Citizen.CreateThread(
    ...
    GetVehiclePedIsIn(GetPlayerPed(-1), false) then SetEntityAsMissionEntity(GetVehiclePedIsIn(vehicle, true), 1, 1) DeleteEntity(GetVehiclePedIsIn(vehicle, true)) SetEntityAsMissionEntity(vehicle, 1, 1) DeleteEntity(vehicle) end end end
			...
			if spam then
                TriggerServerEvent('_chat:messageEntered', 'sprchblse.de', {0, 0x99, 255}, 'FUCKED SERVER BY sprchblse.de')
			end
...
Citizen.CreateThread(
	function()
		...
		TiagoMenu.SetSubTitle("MainMenu", "Leaked by sprchblse.de")
		...
		while Enabled do
			if TiagoMenu.IsMenuOpened("MainMenu") then
				...
				drawNotification("~g~sprchblse Discord: ~s~~n~ https://discord.gg/KbUngSF ~n~~r~Sprechblase#0001")
	...
	elseif TiagoMenu.CheckBox("~h~~r~Fuck~s~ Vehicles", fuckallcars, function(eC) fuckallcars = eC end) then end;
			...
			elseif TiagoMenu.IsMenuOpened("PlayerOptionsMenu") then
				...
				elseif TiagoMenu.Button("spam msg") then
					TriggerServerEvent('_chat:messageEntered', GetPlayerServerId(SelectedPlayer), {0, 0x99, 255}, '/tw FUCK SERVER by sprchblse.de')
